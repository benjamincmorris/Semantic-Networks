...
childes_stemmed_zeroed <- ifelse(childes_stemmed_sim < 0, 0, childes_stemmed_sem)
...
all_cdi_words_stemmed <- read_csv('~/Desktop/cdi_words_stemmed.txt', col_names='uni_lemma')
...
stemmed_and_unstemmed <- bind_cols(data.frame(unstemmed = all_cdi_words), all_cdi_words_stemmed) %>%
  mutate(in_childes_stemmed = uni_lemma %in% rownames(model_wordbank_stemmed),
         in_childes = unstemmed %in% rownames(model_wordbank))

model_wordbank_stemmed <- model_childes_mom[[which(rownames(model_childes_stemmed) %in% 
...
plot_me <- connected_both_SWOW
title <- expression(connected_both_SWOW)
logspace <- function(d1, d2, n) exp(log(10)* seq(d1, d2, length.out=n))

min_degrees <- logspace(0, log10(max(plot_me$degree)), 100)
