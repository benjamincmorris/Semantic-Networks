...
attachment_nodes <- c('hi', 'cow', 'tiger', 'farm', 'giraffe')
# attachment_nodes <- sixteen_month_words$uni_lemma[1:17]
...
for (i in 1:3) {
    for (i in 1:(length(all_items)-length(attachment_nodes))) {
        ...
        node_nums <- (rownames(wordbank_normal) %in% attachment_nodes)
        ...
        # matrix multiplication to get subset weights
        matrix_a <- flipped_nodes %*% wordbank_normal
        matrix_c <- matrix_a * flipped_nodes
        
        #sequential
        chosen_node <- sample(1:length(matrix_c), 1, prob = matrix_c)
        
        chosen_matrix <- rep(0, length(matrix_c))
        chosen_matrix[chosen_node] <- 1
        
        sequential_add_probability <- t(to_add) * (t(chosen_matrix) %*% wordbank_normal) 
        
        #simultaneous
        simultaneous_add_probabiltiy <- t(to_add) * (matrix_c %*% wordbank_normal)
        ...
        preferential_acquisition_add_probabiltiy <- t(to_add) *  rowSums(wordbank_normal)
        all_items[sample(1:655, size = 1, prob = preferential_acquisition_add_probabiltiy)]

        #matrix_b <- wordbank_normal %*% nodes
...
attachment_nodes2 <- c('woof woof', 'hi', 'cow', 'tiger', 'farm', 'giraffe', 'sticky')
    node_nums <- which(rownames(wordbank_normal) %in% attachment_nodes2)
    wordbank_normal[node_nums, node_nums]
    rowSums(wordbank_normal[node_nums, node_nums])
