...
# full childes
# model = train_word2vec("data/CHILDES_words_only.txt",output="full_vectors.bin",
#        threads = 4,vectors = 100, window=6, cbow=1, min_count = 10, force= TRUE)

model = read.vectors("data/full_vectors.bin")
...
SWAPPING WORD ORDER TO EXAMINE EFFECTS ON MODEL
```{r, echo=FALSE}
ordered_words
...
```{r, echo= false}
###### building ordered networks probablistically ##########
...
# but wouldn't that privledge the case where a possible node only connects to one high degree node
...
attachment_degree <- c(0,0)
all_attach_nodes <- c(1:655)
...
###SEQUENTIAL###########
get_seq_nodes <- function(starting_nodes, num_words_to_add, initial_degree) {
        if (num_words_to_add == 0) {
              c(starting_nodes, initial_degree)
        } else {
              nodes <- build_current_net_matrix(starting_nodes)
              matrix_c <- ((t(nodes) %*% wordbank_zeroed)) * t(nodes)
              ...
              sequential_add_probability <- t(to_add) * (t(chosen_matrix) %*% wordbank_zeroed) 
              added_node <- sample(1:655, size=1, prob= sequential_add_probability)
              current_nodes <- c(starting_nodes, all_items[added_node])
              num_words_to_add <- num_words_to_add - 1
              net_degree <- c(initial_degree, mean(build_matrix_c(current_nodes)))
              get_seq_nodes(current_nodes, num_words_to_add, net_degree)
        }
}

t1 <- proc.time()
sequential_words_degree <- replicate(20000, 
                 get_seq_nodes(attachment_nodes, length(all_items) - length(attachment_nodes), 0))
proc.time() - t1
...
# Building a list of words that maximizes local degree using matrix multiplication
for (i in 1:3) {
    for (i in 1:(length(all_items)-length(attachment_nodes))) {
        #make a matrix that marks 0/1 all words as absent/present
        node_nums <- (rownames(wordbank_zeroed) %in% attachment_nodes)
        nodes <- ifelse(node_nums==TRUE, 1, 0)
        nodes <- as.matrix(nodes)
        rownames(nodes) <- all_items
        flipped_nodes <- t(nodes)
        to_add <- ifelse(nodes==1, 0,1)
        
        # matrix multiplication to get weights for nodes in the network at this step
        matrix_a <- flipped_nodes %*% wordbank_zeroed
        matrix_c <- matrix_a * flipped_nodes
        ...
        sequential_add_probability <- t(to_add) * (t(chosen_matrix) %*% wordbank_zeroed) 
        added_node <- sample(1:655, size=1, prob= sequential_add_probability)
        # this adds added_node and calculates equivalent of mean(graph.strength) for the net
        (sum(matrix_c) + sequential_add_probability[added_node]) / sum(nodes)
        
        
        ###SIMULTANEOUS################
        simultaneous_add_probabiltiy <- t(to_add) * (matrix_c %*% wordbank_zeroed)
        
        ###LURE OF THE ASSOCIATES###########
        loa_add_probability <- t(to_add) * matrix_a 
        

        ###PREFERENTIAL ACQUISTION#########
        preferential_acquisition_add_probabiltiy <- t(to_add) *  rowSums(wordbank_zeroed)
        all_items[sample(1:655, size = 1, prob = preferential_acquisition_add_probabiltiy)]

        #matrix_b <- wordbank_zeroed %*% nodes
        #degree_nodes <- matrix_b %*% matrix_a %*% to_add
        # the node values still haven't been zeroed for nodes already in the network
        #   Can't seem to do this with the matrix multiplier func %*% but can with simple *
        degree_new_nodes <- to_add * degree_nodes
        new_node <- sample_n(as.data.frame(degree_new_nodes), 1, weight=V1)
        attachment_nodes <- c(attachment_nodes, rownames(new_node))
        attachment_degree <- c(attachment_degree, new_node$V1)
    }
    all_attach_nodes <- cbind(all_attach_nodes, attachment_nodes)
    attachment_nodes <- c('these','those')
}


    # my initial attempt, no matrix algebra
attachment_nodes2 <- c('woof woof', 'hi', 'cow', 'tiger', 'farm', 'giraffe', 'sticky')
    node_nums <- which(rownames(wordbank_zeroed) %in% attachment_nodes2)
    wordbank_zeroed[node_nums, node_nums]
    rowSums(wordbank_zeroed[node_nums, node_nums])
    degree_i <- summed_degree[node2]
