...
ggplot(data=random_degree_avg, x=num_words, y=mean) +
  ...
  #geom_pointrange(data=simultaneous_degree_avg, aes(x=num_words, y=mean, 
...
sequential_words <- 
    sequential_words_degree[(1:(nrow(sequential_words_degree)/2)),]
all_items[1]

get_word_positions <- function(data_frame, how_many_words)
    if(how_many_words==0) {
          ...
          word_orders <- c(word_orders, i, word_positions_fixed)
          this_many_more_words <- how_many_words - 1
          get_word_positions(data_frame, this_many_more_words)
    }
