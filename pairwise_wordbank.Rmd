...
tmp_fixed <- tmp %>% filter(!is.na(referent)) %>% ungroup() %>% group_by(subj, age) %>%
  summarize(referents=paste(referent, collapse=' '), n=n()) %>% 
  as.data.frame()

tmp_window <- tmp %>% filter(!is.na(referent)) %>% ungroup() %>%
  mutate(run=rep(1:(nrow(.)/150), each=150)) %>% 
  group_by(subj, age, run) %>%
  summarize(referents=paste(referent, collapse=' '))
# # ------------------------------

hm <- VectorSource(unlist(lapply(tmp_fixed$referents, as.character)))
